...
**Bẫy đã biết:** `npm run verify` tại máy chạy `build`, và `build` ghi lại `plugin/shared/version.ts` từ `package.json`. Sau khi bump, phải chạy `verify` **rồi mới** commit, nếu không `version.ts` trong commit còn số cũ và plugin sẽ tự báo sai phiên bản.
