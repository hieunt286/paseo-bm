...
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | Phase 2a-16 xong (beads `bm-phase-2a-16-fallback-worker-332y.1` → `.12`): dòng 10–12 của PRD delta §4 đã áp vào PRD gốc; errata §2.6, §3.1, §8 vào thiết kế gốc; GUIDE thêm `role-fallback.json`, `role-fallback-state.json` và alias `bm-worker-fallback-<n>`; checklist nghiệm thu phần 2a-16 và ghi chú phát hành `docs/operations/paseo-bm-release-notes-0.3.0-alpha.2.md`. Khi implement: Q16 a (mẫu regex lồng lượng từ bị bỏ); errata thiết kế delta §4.4.11; `.4` và `.7` do sub-agent viết, Worker review và chạy kiểm |
