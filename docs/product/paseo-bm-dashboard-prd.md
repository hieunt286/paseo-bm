...
| REQ-060 | Tab "Beads" trong menu "+", Setup là màn chính của Beads Manager, màu trạng thái cho bead, danh sách theo nhóm | P2 | (Delta [`prd-delta-20260918e-beads-tab`](paseo-bm-prd-delta-20260918e-beads-tab.md), owner chốt Q1–Q12 và batch `b5`; (g), (h) sửa và (j)–(m) thêm ở batch `b4`; (m) mở rộng và (n) thêm ở batch `b5`.) (a) Menu "+" của thanh tab workspace có đúng một mục mới "Beads" (nhóm của plugin, trước "Beads agents"), mở một tab của chính workspace đó, trên desktop và trong màn "New tab" của mobile. (b) Tab có hai tab con Beads và Metric, mở ra ở Beads; nội dung là màn Beads và màn Metric hiện có; không nút ←, không tên workspace. (c) Xem được ở bố cục hẹp (`layout.compact`): không gì bị cắt mất chức năng, cuộn dọc được; tab con đọc được bằng trình đọc màn hình (vai trò tab, trạng thái chọn). (d) Surface "Beads Manager" mở ra ở Setup (công cụ beads, agent skills, chỉ dẫn thêm từng vai), tiêu đề "Beads Manager", không nút ←, dòng phiên bản ở cuối. (e) Nút "Workspaces" mở danh sách workspace như trước (ghim, chấm đang chạy, số bead, Go to / Metric / Beads, lịch sử workspace đã đóng) với ← về màn chính; Metric/Beads mở từ danh sách có ← về danh sách. (f) Không mất lối vào nào (hai mục Command Center, `/bm-worker-new`, `/bm-worker-stop-all`); thông báo của slash command và trạng thái mở Manager hiện ở màn chính và ở danh sách workspace. (g) Trong danh sách của màn Beads và panel "Beads in this chat", chỉ **tên** bead mang màu trạng thái: chưa làm = accent, đang làm = vàng, block = đỏ, đã đóng = xanh lá; dòng không tô, không vạch trái; màu lấy từ theme. (h) Chip trạng thái cùng màu với tên ở mọi chỗ hiện bead (màn Beads, panel "Beads in this chat", chip bead trong thẻ chat); chữ của chip giữ nguyên. (i) Tên bead ở các danh sách đó và ở khung chi tiết mở từ chip không in đậm. (j) Nút mắt ở đầu danh sách màn Beads ẩn/hiện bead đã đóng, kèm số lượng; mặc định ẩn; nhớ trong phiên app, tải lại app thì về ẩn; mọi bead khớp đều đã đóng mà đang ẩn thì danh sách nói rõ. (k) Danh sách màn Beads chia bốn nhóm In progress → Blocked → Ready → Closed, mỗi nhóm có vạch ngăn và tên kèm số lượng; trong nhóm giữ cách sắp đang chọn; bộ lọc áp trước; nhóm rỗng không hiện; giới hạn 200 dòng tính theo thứ tự nhóm. (l) Dòng đầu màn Beads, cạnh Refresh, hiện `✓ <đã đóng> / <tổng> done`, đếm như thẻ Progress (không đếm epic, không theo bộ lọc). (m) Không còn biểu đồ "Created / closed, last 14 days" ở màn Beads, và tổng quan màn Beads không còn tính số tạo/đóng theo ngày. (n) Trên dòng workspace của danh sách Workspaces, số bead đang làm màu vàng và số bead block màu đỏ khi lớn hơn 0 (cùng màu với tên bead); số 0 vẫn xám; tổng số bead và số Worker đang chạy giữ màu cũ. |
