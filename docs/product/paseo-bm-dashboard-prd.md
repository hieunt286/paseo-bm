...
| Tài liệu sống | Từ 2026-09-25 PRD này được **sửa tại chỗ** và luôn tả yêu cầu hiện hành, kèm một dòng Revision History cho mỗi lần sửa; git là vết kiểm tra. PRD nói **kết quả người dùng nhận được**; tên nút, màu theme, hằng số, giới hạn dòng và bố cục nằm ở [Technical Design](../design/paseo-bm-dashboard.md), nên chỉnh giao diện không phải là sửa một yêu cầu đã duyệt |
...
| Technical Design | [paseo-bm — Dashboard điều phối](../design/paseo-bm-dashboard.md) (Active, tài liệu sống) |
| ADR | [ADR-007](../adr/ADR-007-dashboard-trace-store.md) — lưu vết trace trong thư mục cài đặt, người dùng xoá được (Accepted) |
...
| D-10 | Token và chi phí | Mọi số tiền mang nhãn "tạm tính" kèm ngày của bảng giá; model không có giá thì **chỉ** hiện token, không hiện tiền; và **không** con số nào cộng dồn chi phí luỹ kế của phiên mà provider báo |
...
|---|---|---|---|
| REQ-040 | Lối vào Dashboard | P2 | (a) Từ màn hình "Beads Manager" mở được Dashboard (màn Metric) của từng workspace, trên cùng surface, mà không mất lối mở chat với Beads Manager. (b) Có một mục Command Center theo ngữ cảnh workspace mở thẳng Dashboard của workspace đang mở. (c) Máy chưa có workspace nào, hoặc workspace chưa có dữ liệu nào, thì hiện trạng thái rỗng kèm một câu giải thích, không phải lỗi. (d) Dashboard mở được cả trên host Paseo không cho plugin mở agent; khi đó các link "mở agent" bị ẩn và có một dòng giải thích, phần còn lại vẫn dùng được. |
| REQ-041 | Danh sách trace theo yêu cầu | P2 | (a) Một dòng = **một yêu cầu người dùng gửi cho Manager**; báo cáo của Worker gửi về Manager không bao giờ là một dòng riêng. (b) Mỗi dòng có: thời điểm nhận, trích yêu cầu một dòng, `requestId` khi biết, trạng thái (`đang chạy` / `hoàn tất` / `đang chờ người dùng` / `đã dừng` / `lỗi` / `không rõ`), số Worker, số agent Reviewer, số lượt gọi review, tổng thời gian, số bead tạo/cập nhật/đóng, và token kèm chi phí. (c) Sắp xếp mới nhất trước, phân trang, có trần số trace đọc mỗi lần (REQ-049) và nút tải thêm. *(Chưa làm: nút tải thêm; hiện chạm trần thì màn hình chỉ nói còn yêu cầu cũ hơn không hiện.)* (d) Mỗi dòng ghi **độ chắc chắn của việc nhóm**: `khớp requestId` / `suy luận` / `không rõ`; không bao giờ trình bày suy luận như dữ kiện. |
...
| REQ-049 | Trần, phân trang và chịu lỗi | P2 | (a) Có trần cho số trace mỗi trang, số entry timeline đọc mỗi agent mỗi lần, và kích thước `issues.jsonl` đọc một lần (giá trị ở Technical Design). Chạm trần thì hiện "còn dữ liệu cũ hơn" kèm nút tải thêm, không âm thầm cắt. *(Chưa làm: nút tải thêm; lời báo "còn dữ liệu cũ hơn" đã có.)* (b) Agent đã lưu trữ hoặc đã bị xoá → hiện phần đã lưu vết kèm ghi chú. (c) Timeline bị Paseo thay thế hoặc có khoảng trống (`reset`, `gap`) → đọc lại và ghi "dữ liệu có thể thiếu". (d) RPC lỗi → thông báo kèm mã lỗi, phần còn lại của màn hình vẫn dùng được. |
...
| REQ-051 | Nhãn `bm.requestId` khi tạo agent | P2 | (a) `roles/manager.md` yêu cầu Manager gắn nhãn `bm.requestId` khi tạo Worker; `roles/worker.md` yêu cầu Worker gắn `bm.requestId` và `bm.batchId` khi tạo Reviewer. (b) Phiên bản payload và phiên bản chỉ dẫn được bump theo, và `bm.version` của agent mới phản ánh bản mới. (c) Agent tạo bởi bản cũ (không có nhãn) vẫn nhóm được ở mức "suy luận"; agent mới nhóm ở mức "khớp requestId". (d) Nhãn thiếu hoặc sai định dạng không bao giờ làm mất trace. |
| REQ-052 | Token và chi phí | P2 | (a) Mỗi trace và mỗi agent trong trace hiện: token vào, token vào từ cache, token ra, và chi phí. (b) Chi phí một request luôn là số **tạm tính** từ token của từng lượt; con số chi phí mà provider báo là tổng luỹ kế của cả phiên agent, nên không bao giờ được cộng vào một request. (c) Số tạm tính dùng **bảng giá đi kèm bản phát hành**, tính riêng token cache theo đơn giá cache, và nhãn hiển thị là "tạm tính" kèm **ngày của bảng giá** và model id. (d) Model không có trong bảng giá → **chỉ** hiện token, không hiện tiền. (e) Không bao giờ gọi mạng để lấy giá. (f) Màn hình nêu rõ đây không phải hoá đơn. |
| REQ-053 | Kho lưu vết bền | P2 | (a) Trace được ghi lại tại thời điểm nó xảy ra, nên vẫn đọc được sau reload plugin, khởi động lại daemon, và sau khi agent bị lưu trữ hoặc xoá (D-8). (b) Kho nằm trong thư mục cài đặt của paseo-bm, tách theo workspace, quyền `0600`/`0700`, ghi atomic, và có **phiên bản lược đồ**: bản lược đồ cao hơn mức hiểu được thì đọc ở chế độ hạn chế kèm thông báo, không bao giờ ghi đè. (c) Ghi lỗi (hết đĩa, không đủ quyền) không được làm hỏng agent đang chạy: hook bỏ qua, ghi log một dòng, Dashboard hiện cảnh báo "có thể thiếu trace". (d) Kho không chứa bí mật (REQ-048b) và không chứa nội dung file của repo ngoài những gì agent đã nói. |
...
| REQ-060 | Tab "Beads" của workspace, Setup là màn chính của Beads Manager, cách bead nói trạng thái | P2 | (a) Mỗi workspace mở được một tab "Beads" của chính nó từ menu thêm tab của Paseo (desktop); plugin chỉ thêm đúng một mục vào menu đó. (b) Tab có hai tab con Beads và Metric, mở ra ở Beads, nội dung là hai màn hiện có; tab đã nằm trong workspace nên không có nút quay lại hay tên workspace. (c) Dùng được ở bố cục hẹp: không mất chức năng, cuộn dọc được; tab con đọc được bằng trình đọc màn hình. (d) Surface "Beads Manager" mở ra ở màn Setup của máy (công cụ beads, agent skills, cấu hình từng vai), không có nút quay lại, có phiên bản plugin. (e) Từ Setup mở được danh sách workspace (agent đang chạy, số bead, lối tới chat Manager / Metric / Beads, lịch sử workspace đã đóng), **xếp theo hoạt động gần nhất, không ghim**; mỗi màn con có đường quay về màn trước nó. (f) Không mất lối vào nào (Command Center, `/bm-worker-new`, `/bm-worker-stop-all`); thông báo của slash command và trạng thái mở Manager thấy được ở mọi màn của surface. (g) Trạng thái của bead **đọc được mà không cần phân biệt màu**: chữ nói trạng thái, bead chưa xong đọc rõ, bead đã đóng đọc mờ; ít màu để đỡ mỏi mắt; màu lấy từ theme. (h) Ở mọi chỗ hiện bead, chip trạng thái và tên bead luôn nói cùng một trạng thái. (i) Tên bead không in đậm. (j) Người dùng ẩn/hiện được bead đã đóng và thấy số lượng; mặc định hiện; lựa chọn nhớ trong phiên app; mọi bead khớp đều đã đóng mà đang ẩn thì màn hình nói rõ. (k) Màn Beads xếp bead theo trạng thái (đang làm, bị chặn, sẵn sàng, đã đóng) như REQ-069 (a)–(c), mỗi nhóm có số lượng; bộ lọc và cách sắp đang chọn áp trước; danh sách dài bị cắt thì nói còn bao nhiêu. (l) Đầu màn Beads cho thấy ngay đã xong bao nhiêu trên tổng, đếm như thẻ Progress (không đếm epic, không theo bộ lọc). (m) Màn Beads không có biểu đồ tạo/đóng theo ngày. (n) Trên dòng workspace, con số cần chú ý (bead đang làm, bead bị chặn, Worker đang chạy) nổi hơn con số bằng 0, cùng cách đọc với tên bead, không dùng màu. (o) Trên điện thoại (app mobile của Paseo không có menu thêm tab) và trên desktop, header của mỗi workspace đang mở có lối vào tab "Beads" của chính workspace đó; lối vào theo danh sách workspace đang mở, đọc lỗi thì giữ nguyên. |
| REQ-069 | Kanban theo trạng thái, trạng thái bead không dựa vào màu, Setup chia phần, đếm lỗi ở Metric | P2 | (a) Màn Beads hiện bead thành các cột theo trạng thái — đang làm, bị chặn, sẵn sàng, đã đóng — mỗi cột có số lượng; cột rỗng vẫn hiện và nói rõ là rỗng. (b) Màn rộng hiện các cột cạnh nhau, vừa bao nhiêu cột thì hiện bấy nhiêu trên một hàng; màn hẹp hiện một cột mỗi lần, chọn bằng dãy tab trạng thái có số lượng, đọc được bằng trình đọc màn hình; không chức năng nào mất ở màn hẹp. (c) Cột đã đóng là một cột như các cột khác và hiện sẵn; vẫn ẩn/hiện được (REQ-060 (j)). (d) Ở mọi chỗ hiện bead — màn Beads, tab "Beads", panel "Beads in this chat", chip bead trong thẻ chat, con số trên dòng workspace — trạng thái chỉ nói bằng chữ và độ đậm nhạt của chữ, không bằng màu; tên bead vẫn không in đậm. (e) Ngoài các chỗ hiện bead, lỗi và cảnh báo giữ màu như cũ. (f) Màn Setup chia ba phần xem từng phần một (công cụ beads, agent skills, agents); tình trạng và cảnh báo công cụ nằm ngoài các phần nên không phần nào che được một vấn đề; việc chọn phần đọc được bằng trình đọc màn hình. (g) Màn Metric có con số lỗi: số lần request bị lỗi bất kể lý do (lượt kết thúc thất bại, agent kết thúc ở trạng thái lỗi, sự cố fallback nhà cung cấp), kèm số request bị ảnh hưởng; bằng 0 thì nói rõ chưa ghi nhận lỗi nào; mỗi lần lỗi chỉ đếm một lần, dù request nhiều lượt hỏi và dù nhiều nguồn cùng ghi nhận một lần chết. (h) Sau khi giao việc, đóng hay xoá một bead từ màn Beads, dòng kết quả vẫn còn khi bead chuyển sang cột khác; hộp xác nhận đang mở thì đóng lại. |
...
| Snapshot agent có `createdAt`, `updatedAt`, `lastUserMessageAt`, `activeTurn.startedAt`, `status`, `labels`, `parentAgentId`, `archivedAt`, **`lastUsage`** (`inputTokens`, `cachedInputTokens`, `outputTokens`, `totalCostUsd`, `contextWindowUsedTokens`) | Đủ để dựng cây, biết lượt đang chạy, và hiện token (REQ-052). **Đo trên daemon thật 2026-09-16:** ba trường token là **của từng lượt**, nhưng `totalCostUsd` là **tổng luỹ kế của cả phiên agent** — qua 12 lượt Manager nó chỉ tăng (0,3956 → … → 2,2712) trong khi token bên cạnh lên xuống. Vì vậy chi phí một request **luôn** là số tạm tính từ token, xem [delta 20260916-acceptance-fixes](../archive/design/paseo-bm-delta-20260916-acceptance-fixes.md) vị trí 3 (đổi một phần Q-035) |
...
| Date | Author | Change |
|---|---|---|
| 2026-09-25 | hieu.nt10 | Cập nhật phần đã cũ: REQ-045 (d) theo quyết định ngày 2026-09-25 (mức Nhỏ không bead, không review); ghi rõ "Chưa làm" ở REQ-041 (c), REQ-049 (a), REQ-055 (a) — nút tải thêm và số trace chưa có trong code. Status giữ Accepted |
| 2026-09-25 | hieu.nt10 (soạn bởi Claude) | **Chuyển chi tiết thiết kế sang design; PRD là tài liệu sống.** Các REQ nói kết quả người dùng nhận được: tên token màu, hằng số, giới hạn dòng, nhãn nút, ghi chú "Trước 2026-09-25" và nguồn delta/request/Q trong ngoặc được bỏ khỏi REQ-040, REQ-044, REQ-046, REQ-049, REQ-051 → REQ-053, REQ-055 → REQ-058, REQ-060, REQ-069 (chi tiết đã có trong [Technical Design](../design/paseo-bm-dashboard.md) §11–§16). Errata theo quyết định đã ghi: REQ-052 (b) và D-10 — chi phí luôn tạm tính, không dùng chi phí luỹ kế của provider (delta acceptance-fixes vị trí 3); REQ-045 (a) thêm bước review plan (đã ghi ở dòng 2026-09-17); REQ-057 (e) chống trùng không còn là `(agentId, turnId)`. Header: Technical Design Active, ADR-007 Accepted. Ba PRD delta đã áp (20260918e, 20260918f, 20260925) được đánh dấu Merged. Không đổi phạm vi |
| 2026-09-25 | hieu.nt10 (soạn bởi Beads Worker) | **Áp dụng delta [`prd-delta-20260925-kanban-quiet-colours`](../archive/product/paseo-bm-prd-delta-20260925-kanban-quiet-colours.md)** (owner chốt Q1 a, Q2 a, Q3 a, Q4 a; request `req-20260925T015709Z`): thêm REQ-069 — kanban bốn cột có responsive, trạng thái nói bằng chữ đậm/mờ thay vì màu, ba tab cho màn Setup, thẻ "Errors" ở màn Metric; errata REQ-060 (g), (h), (j), (k), (n) theo đó. (i) "tên bead không in đậm" giữ nguyên. Status giữ Accepted |
| 2026-09-19 | hieu.nt10 (soạn bởi Beads Worker) | **Áp lại delta [`prd-delta-20260918e-beads-tab`](../archive/product/paseo-bm-prd-delta-20260918e-beads-tab.md) sau batch `b6`** (owner chốt Q14 a của request `req-20260918T043115Z`): REQ-060 (a) errata — app mobile của Paseo 0.8 không có "+"; thêm (o) — nút "Beads" trên header của workspace. Status giữ Accepted |
| 2026-09-18 | hieu.nt10 (soạn bởi Beads Worker) | **Áp dụng delta [`prd-delta-20260918f-remove-pinning`](../archive/product/paseo-bm-prd-delta-20260918f-remove-pinning.md)** (owner chốt Q11 c, duyệt Q14 a; request `req-20260918T063746Z`): REQ-060 (e) bỏ "ghim" — danh sách Workspaces xếp theo hoạt động gần nhất, không ghim. Status giữ Accepted |
| 2026-09-18 | hieu.nt10 (soạn bởi Beads Worker) | **Áp lại delta [`prd-delta-20260918e-beads-tab`](../archive/product/paseo-bm-prd-delta-20260918e-beads-tab.md) sau batch `b5`**: REQ-060 (m) — bỏ luôn phần tính số tạo/đóng 14 ngày; thêm (n) — số đang làm và số block trên dòng workspace dùng màu vàng và đỏ. Status giữ Accepted |
...
| 2026-09-18 | hieu.nt10 (soạn bởi Beads Worker) | **Áp dụng delta [`prd-delta-20260918-manager-mode-model-metrics`](../archive/product/paseo-bm-prd-delta-20260918-manager-mode-model-metrics.md)** (owner chốt Q32, Q38): thêm REQ-058 — model, mức thinking và mode của từng agent; token theo model; thẻ model × vai trò; chi phí theo model thật sự chạy. Status giữ Accepted |
