...
*(Errata 2026-09-18, owner chốt Q1a và Q2a của req-20260918T035101Z, chi tiết ở errata §4.6 của [delta context-engineering](./paseo-bm-delta-20260917c-context-engineering.md): vế "Worker thôi chọn mode cho Reviewer (hook hạ `full-access` thừa kế về `auto`)" ở trên **sai** — daemon **từ chối** Reviewer do Worker `bypassPermissions` tạo mà không truyền mode, trước khi hook chạy. Worker giờ truyền mode Reviewer như Manager truyền mode Worker: plugin nối dòng ``Reviewer mode: `<modeId>` — pass it as `settings.modeId` when you create a Reviewer.`` vào mục `## Runtime facts` của Worker, giá trị chọn bằng đúng luật Reviewer của hook (mode tự đặt trên profile `bm-reviewer` thắng, trừ `dangerous`/`planning`). Không có dòng đó thì Worker gửi `blocked` với nguyên văn lỗi của Paseo, không tự chọn. Hook vẫn hạ mode `dangerous`/`planning` của Reviewer.)*
...
*(Errata 2026-09-18, owner chốt Q1a → Q5a của req-20260918T064258Z: ma trận trên **không** chạy ở mỗi commit. CI mỗi commit (`.github/workflows/ci.yml`) tối giản: một job ubuntu, Node 22 (mức `engines` thấp nhất) chạy `npm ci`, typecheck gốc và plugin, lint, test, trên push lên `main` và PR, bỏ qua commit chỉ đụng `docs/**`, `.beads/**`, `README.md`, `GUIDE.md`, `AGENTS.md`, `CLAUDE.md`, `assets/**`. Ma trận đầy đủ macOS + Linux × Node 22 + 24 — typecheck gốc và plugin, lint, test, build, smoke gói đã đóng gói, kiểm tra không có script vòng đời cài đặt — chạy trong `.github/workflows/release.yml` ở cả bản phát hành lẫn lượt chạy tay (dry-run); bước publish chỉ chạy khi cả bốn tổ hợp xanh. Bước publish, quyền OIDC và tên file `release.yml` không đổi.)*
