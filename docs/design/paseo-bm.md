...
- **Ranh giới tin cậy 3 (mới) — mở quyền tạo agent.** Cho Manager và Worker quyền dùng công cụ Paseo nghĩa là cho chúng tạo và dừng agent khác, tức là tiêu tiền và chạy mã. Phải có đồng ý riêng (`--enable-agent-tools`), hẹp nhất có thể theo ADR-006, và `doctor` phải hiển thị vai trò nào đang có quyền.
