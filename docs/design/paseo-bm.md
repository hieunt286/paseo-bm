...
**Switch — Manager** (plugin tạo bằng `createManager`; **(0.4.0)** như Switch của Worker, tool agent tắt → `E_FALLBACK_CREATE_FAILED` với `AGENT_TOOLS_OFF_SWITCH_MESSAGE`, không tạo gì, giữ `pending`, vì Manager thay thế là Manager mà Beads Manager sẽ mở): provider `bm-manager-fallback-<n>/<model>`, mode theo luật Manager kèm `bm.modeSet`, thinking của mục, nhãn `bm.replaces`, prompt:
...
Khi công tắc tắt và workspace chưa có Manager, `manager.ensure` không tạo Manager mà ném `E_PROVIDER_UNAVAILABLE` với `AGENT_TOOLS_OFF_MESSAGE`: `Paseo's agent tools are off, and a Beads Manager created now would never get them, so it could not create a Worker. Press "Allow agent tools…" in Setup, then open Beads Manager again.` Vai trò vẫn được tạo trước đó; khi chính lần gọi này vừa tạo chúng, câu về vai trò (ở trên) đứng trước `AGENT_TOOLS_OFF_MESSAGE` trong thông điệp, vì lần mở sau không còn nói điều đó. Dòng lỗi hiện ngay trên màn Setup, nơi có nút ấy (§8), và dòng trạng thái tool agent trên Setup là "Off — no new Beads Manager starts until you allow them" (Design Dashboard §11.3).
...
| Date | Author | Change |
|---|---|---|
| 2026-09-26 | hieu.nt10 (soạn bởi Claude) | Sau review độc lập các bản sửa 0.4.0: Switch của Manager và của Worker (§7.10) không tạo agent thay thế khi tool agent tắt, giữ sự cố `pending`; hook (§7.2) bỏ `thinkingOptionId` của bên tạo khi đổi model và tra feature theo model của profile |
| 2026-09-26 | hieu.nt10 (soạn bởi Claude) | §7.13.2: khi `manager.ensure` từ chối vì tool agent tắt mà chính lần gọi vừa tạo vai trò, thông điệp giữ câu về vai trò; câu trạng thái trên Setup đổi theo hành vi không tạo Manager |
