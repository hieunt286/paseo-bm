...
| ADR | [ADR-008](../adr/ADR-008-role-settings-written-by-plugin.md) (mới, Proposed); sửa đổi [ADR-004](../adr/ADR-004-paseo-config-mutation.md) QĐ1 và [ADR-006](../adr/ADR-006-role-registration.md) QĐ1, QĐ5. [ADR-007](../adr/ADR-007-dashboard-trace-store.md): chỉ thêm một trường tuỳ chọn |
...
4. `config.get()` lần nữa. Mọi profile có id **không** bắt đầu bằng `bm-` được so với bản của bước 1. Khác → thêm vào `warnings`: `profile "<id>" changed while saving and may have lost that change`. Không ghi lại.
5. Trả `revision` mới.
...
- **Ghi `config.json` từ plugin (ADR-008):**
  ...
  - báo khi một profile khác đổi trong lúc ghi.

  Rủi ro còn lại, owner chấp nhận ở Q3: một thay đổi trong app rơi đúng vào khoảng giữa lần đọc và lần ghi vẫn bị đè.
...
| Ghi đè thay đổi song song trong `agentProfiles` | `revision` và kiểm sau khi ghi (§4.3.4); rủi ro còn lại owner chấp nhận (Q3) |
