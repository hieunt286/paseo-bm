...
| # | Kiểm | Cách làm | Kết quả mong đợi | Kết quả |
|---|---|---|---|---|
| 18.1 | Điều kiện phát hành (REQ-067 c) | Đọc `~/.paseo-bm/role-fallback-state.json` (chỉ đọc) | Ít nhất một sự cố có `signal` và `message` do một provider thật sinh ra, và `class` đúng với lỗi đó theo bộ mẫu (mặc định hoặc của file) | **Đạt** — đọc 2026-09-23 (`req-20260923T040415Z`). File có 1 sự cố: `fb-d3c9430912b2`, `role` `worker`, `signal` `completed`, `message` `"Failed to authenticate. API Error: 401 API key is invalid."` — do Anthropic thật từ chối lượt Claude Code sinh ra, `class` `L4`, `status` `switched`. Không có `~/.paseo-bm/role-fallback.json` nên bộ mẫu áp dụng là `DEFAULT_PATTERNS`. Chạy chính `classifyText` của `plugin/shared/fallback-patterns.ts` trên `message` đó: trả `L4`, mẫu khớp `/\b401\b/i`, `isFallbackClass` true — **trùng** với `class` đã ghi trong file. |
