# Checklist nghiệm thu Phase 1a trên daemon thật — paseo-bm
...
| Date | 2026-09-15 |
| Owner | hieu.nt10 |
| Liên quan | [PRD M-1 → M-9](../product/paseo-bm-prd.md#2-mục-tiêu--chỉ-số-thành-công) · [Plan v2 WP-120](../plans/paseo-bm-implementation-plan-v2.md#2-work-packages) · [Design §9.1, §10](../design/paseo-bm.md) · [ADR-006](../adr/ADR-006-role-registration.md) |
| Bead | `bm-wp-120-b2e.1.1` (soạn checklist) · lượt chạy thật thuộc `bm-wp-120-b2e.1.2` |

Tài liệu này mô tả **cách chạy và cách chấm**; nó không chứa số đo. Mỗi lượt chạy sao chép mục 9 và mục 10 sang một biên bản riêng trong `docs/operations/`. Ngưỡng của M-1 → M-9 lấy nguyên văn từ PRD đã đóng băng; checklist không đặt ngưỡng mới.
...
```bash
export RUN=~/bm-acceptance/20260915/phase1a
...
- [ ] Ghi run CI của commit đó (dùng cho M-6): `gh run list --commit "$(git -C "$REPO" rev-parse HEAD)" --limit 1 > "$EVID/ci-run.txt"`.
- [ ] Đóng gói bản v1: `cd "$REPO" && npm ci && npm pack --pack-destination "$RUN"`, rồi `export TARBALL=$RUN/paseo-bm-0.1.0-alpha.0.tgz`.
...
- [ ] Dựng bản v2 **ngoài repo**, không đổi repo:
  ```bash
  rm -rf "$RUN/v2src" && git clone -q "$REPO" "$RUN/v2src" && cd "$RUN/v2src"
  npm ci && npm version 0.1.0-alpha.1 --no-git-tag-version && npm pack --pack-destination "$RUN"
  export TARBALL=$RUN/paseo-bm-0.1.0-alpha.1.tgz
  ```
...
- [ ] Cài bên cạnh: `ls "$CLEAN_HOME/.paseo-bm/plugin"` có cả `0.1.0-alpha.0` và `0.1.0-alpha.1`; `jq '.versions' "$CLEAN_HOME/.paseo-bm/install.json"` có hai mục, bản mới `active: true`.
...
| M-6 | Tỉ lệ cài thành công trên ma trận hỗ trợ | Run CI của commit đang nghiệm thu (ubuntu, macOS × Node 22, 24; gồm smoke trên gói đã đóng gói có chạy `install` xem trước và `doctor`) | 4/4 job `success` | `ci-run.txt` + link run |
