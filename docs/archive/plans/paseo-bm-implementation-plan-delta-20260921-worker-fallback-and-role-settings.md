...
| Status | **Completed** (2026-09-25) — mọi bead của plan đã đóng; từ nay tài liệu sống thay cho chuỗi delta (xem `AGENTS.md`, mục Process). Trạng thái trước: **Applied** (2026-09-22): phase 2a-13 → 2a-18 xong (WP-287 → WP-309); phát hành npm bản mang "Auto switch" chờ một sự cố thật được mẫu nhận đúng loại (REQ-067 c, owner chốt Q17 b). Trước đó: **Applied — 2a-18 deferred** (0 sự cố thật, Q13 a); trước nữa: **Active** |
...
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | Phase 2a-15 xong (beads `bm-phase-2a-15-roles-and-models-kj1p.1` → `.6`): dòng 7–9 của PRD delta §4 đã áp vào PRD gốc; ADR-008 Accepted, dòng "Sửa đổi bởi" của ADR-004 / ADR-006 bỏ "Proposed"; errata §3.4 và §5 vào thiết kế gốc; checklist nghiệm thu phần 2a-15 và ghi chú phát hành `docs/releases/paseo-bm-release-notes-0.3.0-alpha.1.md`. Khi implement: `roles.settings` trả thêm `providers: string[]` cho trường Provider của form (errata design delta §4.3.2); màn Roles & models do một sub-agent viết, Worker review và chạy kiểm |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | Owner chốt Q16 a (WP-300, bead `bm-phase-2a-16-fallback-worker-332y.4`): mẫu regex của người dùng có lượng từ lồng nhau bị bỏ khi biên dịch; tiêu chí "mẫu bệnh lý < 1 giây" của bead ghi rõ điều đó. Cắt 2.000 ký tự chỉ chặn mẫu chi phí bình phương |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | WP-301 (bead `bm-phase-2a-16-fallback-worker-332y.6`), theo Q14 a: trần `manager.md` nâng 177 → 179 (đo thật: thêm đúng 2 dòng cho `BM-FALLBACK`; không đoạn nào của `manager.md` còn chỗ gói lại). Lý do ghi trong `test/roles-content.test.ts` |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | WP-302 (bead `bm-phase-2a-16-fallback-worker-332y.8`), theo Q14 a: trần `worker.md` nâng 397 → 402 (đo thật: đoạn `BM-HANDOVER` 5 dòng; không đoạn nào của `worker.md` còn chỗ gói lại). Lý do ghi trong `test/roles-content.test.ts` |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | WP-303 (bead `bm-phase-2a-16-fallback-worker-332y.11`), theo Q14 a: trần `worker.md` nâng 402 → 403 (đo thật: 1 dòng cho `BM-RESUME`; gói lại cùng đoạn bàn giao không bớt được dòng nào). Lý do ghi trong `test/roles-content.test.ts` |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | Phase 2a-16 xong (beads `bm-phase-2a-16-fallback-worker-332y.1` → `.12`): dòng 10–12 của PRD delta §4 đã áp vào PRD gốc; errata §2.6, §3.1, §8 vào thiết kế gốc; GUIDE thêm `role-fallback.json`, `role-fallback-state.json` và alias `bm-worker-fallback-<n>`; checklist nghiệm thu phần 2a-16 và ghi chú phát hành `docs/releases/paseo-bm-release-notes-0.3.0-alpha.2.md`. Khi implement: Q16 a (mẫu regex lồng lượng từ bị bỏ); errata thiết kế delta §4.4.11; `.4` và `.7` do sub-agent viết, Worker review và chạy kiểm |
| 2026-09-22 | hieu.nt10 (soạn bởi Beads Worker) | WP-305 (bead `bm-phase-2a-17-fallback-reviewer-manager-fnnc.1`), theo Q14 a: trần `worker.md` nâng 403 → 408 (đo thật: đoạn Reviewer dừng vì lỗi provider 4 dòng và một dòng trống; đoạn riêng vì là chủ đề riêng). Lý do ghi trong `test/roles-content.test.ts` |
