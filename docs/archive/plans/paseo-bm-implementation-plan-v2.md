...
- **Outcome:** Ghi `agents.providers.bm-manager|bm-worker|bm-reviewer` (chỉ `extends`, `label`, `paseoTools`) và thêm/sửa **chỉ** các mục `bm-*` trong mảng `daemon.agentProfiles`; **báo trước rồi bật `daemon.mcp.injectIntoAgents`** kèm cảnh báo nêu rõ điều đó cho **mọi** agent trên máy quyền tạo và dừng agent khác (gộp **một lần hỏi** với việc bật plugin; không tương tác thì cờ `--enable-plugins` phủ cả hai — ADR-006 quyết định 3, 4); ghi vào hồ sơ nếu chính paseo-bm bật; backup trước khi ghi; reload và xác minh.
...
| Date | Author | Change |
|---|---|---|
| 2026-09-15 | hieu.nt10 (soạn bởi Claude) | Áp dụng delta `plan-v2-delta-20260915-m10` do owner duyệt: ngưỡng M-10 mà điều kiện ra của WP-117 dẫn chiếu đổi thành ≤ 60 giây theo PRD; nội dung WP không đổi |
| 2026-09-15 | hieu.nt10 (soạn bởi Claude) | **Errata khi implement, không đổi phạm vi.** WP-107 outcome còn nhắc cờ `--enable-agent-tools`, trái với ADR-006 quyết định 3, 4 và Design §4.2 đã gộp vào `--enable-plugins`; sửa lại cho khớp. Ghi nhận lỗ hổng phân rã: bước cấu hình vai trò, đăng nhập provider và đăng ký `bm-*` thuộc outcome WP-106/WP-107 nhưng chưa có bead nối vào luồng install → thêm bead `bm-wp-107-2r5.4` (Split-from `bm-wp-105-6ec.4`) |
