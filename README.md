...
`paseo-bm` adds a small agent team to [Paseo](https://paseo.sh). You describe a change in chat, and the team turns it into documents (when needed), beads and working code. A separate agent reviews each stage, and screens inside Paseo show what happened, the workspace's beads, and whether the machine is set up.

```text
 you ──chat──▶ Beads Manager ──delegates──▶ Beads Worker ──each batch──▶ Reviewer
                    ▲                            │
                    └──────── BM-REPORT ─────────┘

 Paseo sidebar ▸ Beads Manager ▸ [Go to]  [Metric]  [Beads]      ⚙ Setup
```

| Part | What it does |
|---|---|
| **Beads Manager** | Your single point of contact in a workspace. It does not do the work: it hands each request to a Worker right away, tracks the Worker's reports and tells you the result. |
| **Beads Worker** | Carries one request from start to finish with the feature-workflow process: sizes it, writes documents only when the size requires them, creates or updates beads, implements them, runs the checks and closes each bead with evidence. |
| **Reviewer** | Reads one stage of the work and returns a verdict. It never edits anything. |
| **Metric screen** | What each request turned into: agents, messages, tokens, estimated cost, duration, beads, workflow steps and skills used. |
| **Beads screen** | The workspace's beads with overview charts, filters and a readable detail view. From a bead you can hand it to a Worker. |
| **Setup screen** | Whether `br`, `bv` and the agent skills are installed, and your own additional instructions for each role. |
| **Chat cards** | Messages between the agents shown as compact cards, with the beads they name and a reply box. |
| **`npx paseo-bm`** | The installer: copies the plugin, registers it and the three roles with Paseo, installs a missing `br` / `bv`, and can install the recommended agent skills. It also has `doctor` and `uninstall`. |
...
> **Run it, don't add it as a dependency.** `paseo-bm` is a command-line tool you run with `npx`; it exports nothing to `import`. The `npm i paseo-bm` box on the npm website is shown for every package. Running it inside a project only adds a useless dependency.
...
1. Open the Paseo app so its daemon is running.
2. In a terminal, run:
   ...
   Pick a provider and model for each role, read the preview, answer **Yes** to apply, and answer the trust question (see the [warnings](#before-you-install-read-these-warnings)). A missing `br` or `bv` is installed as part of applying.
3. When the summary shows the plugin as `running`, open **Beads Manager** in Paseo's sidebar and press **Go to** next to your workspace.
4. Type what you want, for example: *"In `src/invoice.py`, add a short docstring to `format_date`."*
5. Watch the progress in chat, or press **Metric** and **Beads** on the Beads Manager screen.
